...
We use the following code to provide a summary of our results. The output is a bar chart showing each combination of lamdba and kappa values, along with the ratio group they are assigned to. The y-axis show the proportion that contain 20 generations or have greater than 1000 offspring. There is a clear correlation between the ratio value and the proportion of families containing 20 generation or resulting in more than 1000 offspring. The greater the ratio, the greater the proportion.
...
Another test we used to measure the proportion of simulations with 20 or more offspring is shown below. The output is a bar chart showing each combination of lamdba and kappa values, the ratio group they are assigned to. The y-axis show the proportion that contain 20 or more offspring. There is a clear correlation between the ratio value and the proportion of families containing 20 or more offspring. The greater the ratio, the greater the proportion. While the bar charts look almost identical, they are using different as they show different relationships based on generations and offspring.
...
As may be seen from the above two bar charts, which are meant to identify two different segments of data within our simulations, there is not a visible difference in proportions between the two. To further explore this, we produce a dumbell plot depicting the raw "count" out of 400 replicates which meet each critera for our 30 parameter tests. With Green text depicting the count of replicas which meet "20 Generations or 1000+ Offspring" and Blue "20+ Offspring", there is large overlap in matching criteria. In the event the number of matching replicates does not equal one another, you will find a bar between the two colored points for the test. These dumbell bars provide a great visual insight into which tests accounted for variance, and when the tests stopped varying amongst these two criteria. With the exception of our first ratio group, given a zero count, we found that as the lambda/kappa ratio increased up to 0.7 (ratio group 03) the dumbells close off completely. From ratio group 04 onward, the results between the two comparing demographics merge and the gap in replicates with "= 20 generations or > 1000 offspring" and those ">=20 offspring" approaches zero.
...
Most branching processes that terminated naturally did not surpass more than roughly five generations or created more than ten offspring. This obvious deviation between self-terminating processes and processes with exponential growth ultimately supports using ratio calculations for parameter combinations to produce useful analytic results for gaining deeper insight into these processes. In our final results, we also found that as the lambda/kappa ratio increased to a ratio of 0.7 and greater, with the exception of our first ratio group, the dumbbells closed off completely. To that end, from our fourth ratio group and onward results between the two demographics merged, with the gap replicating again at 20 generations or greater than 1,000 offspring, and those with 20 or more offspring approached zero. These clear deviations between processes ultimately suggests that re-parameterization is indeed appropriate. In addition, no evidence was found to indicate that the magnitude of lambda and kappa have any real impact on the behavior of the branching processes, whereas the lambda/kappa ratio appear to have a significant impact on simulation results from each parameter combination. As an extension of this effort, future works would include additional visualizations, such as contour heat maps and box plots, for further evidence in support of or against this conclusion. 
