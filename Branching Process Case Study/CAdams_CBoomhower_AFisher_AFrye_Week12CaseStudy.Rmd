...
## Abstract
In this case study, we conduct a simulation study of the Aldous and Krebs branching process in order to investigate whether re-parameterization of lambda and kappa as a ratio is appropriate or not for capturing properties useful for interpreting different parameter combinations. To do this, we designed several simulations for evaluating outcomes using parameter combination groupings. Also, the Aldous and Krebs process was simulated in R statistical language via the Monte Carlo method. Initial results using bar plot visualizations indicated no visible difference in proportions and that the lambda/kappa ratio was sufficient; however, dumbbell plot visualizations provided greater insight into which simulation tests accounted for variance. In our final results, we found that as the lambda/kappa ratio increased to a ratio of 0.7 and greater, with the exception of our first ratio group, the dumbbells closed off completely. To that end, from our fourth ratio group and onward results between the two demographics merged, with the gap replicating again at 20 generations or greater than 1,000 offspring, and those with 20 or more offspring approached zero. While these clear deviations between processes seem to show that re-parameterization is appropriate, we found that simply using the lambda/kappa parameter ratio is ultimately the most appropriate and useful solution as it accounts for the highest amount of variation. In addition, no evidence was found to indicate that the magnitude of lambda and kappa have any real impact on the behavior of the branching processes. 
...
Although dumbbell plots ultimately provided better visuals of ratios for different parameter combinations, final results revealed the magnitude of lambda and kappa play a small role whereas the lambda/kappa ratio has a much more significant effect. As indicated in the earlier two bar charts, which aimed to identify two different segments of data within our simulations, no visible difference in proportions was observed and the lambda/kappa ratio was found to be sufficient. As an extension of this effort, future works would include additional visualizations, such as heat maps and box plots, for further evidence in support of or against this conclusion. 
