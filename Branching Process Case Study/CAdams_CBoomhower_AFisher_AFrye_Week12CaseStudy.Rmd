...
mcGridAlive.plot <- ggplot(data=mcGridAlive.df, aes(x=test.names, y=mcGridAlive, fill=ratio.groups,
    ...
    theme(plot.title = element_text(size = 10), axis.text.x = element_text(angle = 90), legend.title = element_text(size = 10))

ggplotly(mcGridAlive.plot)
```
...
ggplotly(mcGridProp20kids.plot)
```
...
dumbell.df <- as.data.frame(cbind(test.names, mcGridAlive, mcGridProp20kids))

# plot_ly(dumbell.df, color = I("gray80")) %>%
#   add_segments(x = ~mcGridAlive, xend = ~mcGridProp20kids, y = ~test.names, yend = ~test.names, showlegend = FALSE) %>%
#   add_markers(x = ~mcGridAlive, y = ~test.names, name = "mcGridAlive", color = I("pink")) %>%
#   add_markers(x = ~mcGridProp20kids, y = ~test.names, name = "mcGridProp20kids", color = I("blue")) %>%
#   layout(
...
gg <- ggplot(dumbell.df, aes(x=mcGridAlive, xend=mcGridProp20kids, y=test.names, group=test.names)) +
        scale_y_discrete(limits=mcGridProp20kids.df$test.names) +
        geom_dumbbell(color="#a3c4dc", 
                      size=0.75, 
                      point.colour.l="#0e668b") + 
        scale_x_discrete(label=mcGridProp20kids) + 
        labs(x=NULL, 
             y=NULL, 
